---
title: "Python SDK"
description: "Own the Dreamscale policy lifecycle from connection through prediction, action buffering, and deterministic cleanup."
keywords: ["dreamscale.connect", "dreamscale.aconnect", "predict", "next_action", "policy.run"]
---
...
sequenceDiagram
  ...
  participant SDK as "Dreamscale policy"
...
<Note>
  ...
  `dreamscale sessions list`, and terminate an abandoned session with
...
| `run()` | A `RunResult` after a bounded loop | Dreamscale owns refill timing; your callbacks own sensing and actuation |
...
```python
import dreamscale
from dreamscale.policy import RunContext
...
Dreamscale stretches the action horizon to match rather than discarding actions,
...
Whether `control_hz` paces anything depends on which interface you chose above:

- `run` owns the clock. It sleeps to each action's deadline, so the rate you set
  is the rate it executes at.
- `next_action`, and any other caller-owned loop, leaves the clock to you.
  `control_hz` is then a declaration the action scheduler plans against — it
  sizes the replan threshold from it — so set it to the rate your loop actually
  achieves. Nothing corrects a wrong one, and the run will still look healthy.

Nothing server-side takes a rate. The model always emits its chunk at the native
rate; `control_hz` only decides how fast the client plays it out.
...
- `keep_warm` asks Dreamscale to hold the session ready between calls instead of
...
with dreamscale.connect(
    model="molmoact2-so101",
    on_progress=lambda message: logger.info("dreamscale: %s", message),
) as policy:
    ...
```
...
overrides. Keep each on `"auto"` unless Dreamscale support asks you to test a
specific configuration:

```python
policy = dreamscale.connect(
    model="molmoact2-so101",
    acceleration="auto",
    control="auto",
    rtc="auto",
...
`dreamscale.so101.observe()` and connect through `dreamscale.connect()`. Existing
