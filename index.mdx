...
Dreamscale runs robot policies in the cloud while your local controller keeps
ownership of sensing, actuation, and safety.

```mermaid
flowchart LR
  A["Robot cameras and state"] --> B["Dreamscale Python SDK"]
  B --> C["Cloud policy"]
  C --> D["Action chunk"]
...
<Note>
  ...
  usage in the
  [dashboard](https://app.dreamscalelabs.com/dashboard).
</Note>
