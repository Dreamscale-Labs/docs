...
keywords: ["Franka", "MolmoAct2 DROID", "dreamscale.franka.observe", "inference only"]
---

Dreamscale exposes MolmoAct2-DROID inference through
`dreamscale.franka.observe()`. You provide camera frames and robot state; Dreamscale
...
dreamscale login
dreamscale doctor
dreamscale status --model molmoact2-droid
```
...
```mermaid
flowchart LR
  A["Exterior RGB"] --> D["dreamscale.franka.observe"]
...
6. a physical e-stop independent of the Dreamscale process;
...
Dreamscale's supported public surface ends at the returned action chunk.
