...
installs a matching `dropbear[dreamzero]` for you.

<Note>
  Dreamscale was previously called Dropbear. The current adapter release is
  still pinned to an earlier SDK that only has the `dropbear` command and reads
  credentials only from `~/.dropbear`. Until a new adapter release ships, use the
  commands on this page exactly as written: `dropbear login`,
  `dropbear status`, `dropbear sessions`, and `--policy dropbear`. Run
  `dropbear login` inside the adapter's environment (for example
  `uv run dropbear login` in the project where you added the adapter). A
  separate `dreamscale login` writes `~/.dreamscale`, which the adapter does not
  read.
</Note>
