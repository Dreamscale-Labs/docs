...
([source](https://github.com/Dreamscale-Labs/inspect-robots-dropbear), Apache
...
<Note>
  **Older adapter (`inspect-robots-dropbear`).** An existing install of the
  adapter under its earlier name keeps working as it is: keep using its own
  commands (`dropbear login`, `--policy dropbear`) until you switch. To switch,
  replace the dependency with `inspect-robots-dreamscale`. Your existing login is
  read in place, and `--policy dropbear` still selects the adapter with its
  earlier artifact names (`dropbear_telemetry`, `dropbear/…` sidecars).
</Note>
...
```bash
--policy dreamscale -P model=dreamzero-yam -P control_hz=15
```
